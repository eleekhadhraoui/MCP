...
def setup_mcp_server(kali_client: KaliToolsClient) -> FastMCP:
    ...
    def dirb_scan(url: str, wordlist: str = "/usr/share/wordlists/dirb/common.txt", additional_args: str = "") -> Dict[str, Any]:
        ...
        result = kali_client.safe_post("api/tools/dirb", data)
        return {"result": result}
    ...
    @mcp.tool()
    def sqlmap_scan(url: str, data: str = "", cookie: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
    ...
    @mcp.tool()
    def wpscan_analyze(url: str, api_token: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute WPScan WordPress vulnerability scanner via the MCP API.

        Args:
        url: The target WordPress URL (e.g., http://192.168.145.128:8080)
        api_token: Optional WPScan API token. If empty, function will try env var WPSCAN_API_TOKEN.
        additional_args: Any extra WPScan CLI args (e.g., "--enumerate u,ap,at")

        Returns:
        The MCP/kali_client response (dict).
        """
    # prefer explicit arg, otherwise use env var
    ...
    # send to your backend endpoint that runs the command on the Kali VM
